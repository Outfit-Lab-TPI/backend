...
public class ProcessSubscriptionPayment {
    ...
    public void execute(String paymentId) {
        ...
        // Extraer información desde externalReference (formato: "PREMIUM_MONTHLY_user@example.com")
        ...
        if (userEmail == null || userEmail.isEmpty()) {
            throw new PaymentFailedException("No se pudo obtener el email del usuario desde externalReference: " + externalReference);
        }

        // Buscar usuario (usar método legacy que maneja la excepción)
        UserModel user;
        try {
            user = userRepository.findUserByEmail(userEmail);
        } catch (UserNotFoundException e) {
            throw new PaymentFailedException("Usuario no encontrado: " + userEmail);
        }
    ...
    /**
     * Extrae el email del usuario desde la referencia externa.
     * Formato esperado: "PREMIUM_MONTHLY_user@example.com"
     */
    private String extractEmailFromReference(String externalReference) {
        ...
        // Buscar el segundo underscore para obtener el email después
        int firstUnderscore = externalReference.indexOf('_');
        if (firstUnderscore > 0) {
            int secondUnderscore = externalReference.indexOf('_', firstUnderscore + 1);
            if (secondUnderscore > 0 && secondUnderscore < externalReference.length() - 1) {
                return externalReference.substring(secondUnderscore + 1);
            }
        }
